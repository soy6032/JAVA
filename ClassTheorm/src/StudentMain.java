...
//		Person 클래스를 상속받아 Person 클래스의 모든 멤버를 사용
...
//		변수의 속 알맹이는 자손 클래스인 Student 임
//		p.name의 값이 Student 클래스 객체의 값이 출력됨
		System.out.println(p.name);
		
//		Person 클래스 타입의 변수 p에 자손 클래스 타입의 객체가 들어 있으므로 p.grade, p.department 가 동작될 것이라 생각하지만 부모 클래스 객체로 잠시 모습이 변해있는 형태이기 때문에 자손 클래스 타입의 멤버들을 사용할 수 없음
//		p.grade = "A";
//		p.department = "Com";
	}
